...
#  where UPLOADED FILES (resumes) are stored, and the big REST_FRAMEWORK block
#  (authentication, permissions, pagination, throttling — including the special
#  'apply' anti-spam throttle).
# ============================================================================
from pathlib import Path

# BASE_DIR is the absolute path to the "Project 5" folder (two levels up).
...
# it in production.
SECRET_KEY = 'django-insecure-jobportal-learning-key-change-me-in-production'
...
INSTALLED_APPS = [
    ...
    'django_filters',               # The ?location=&jtype= filtering backend
    'drf_spectacular',              # Auto-builds the Swagger API documentation

    # --- Our own app ---
    'jobs',                          # Profiles, Jobs and Applications live here
]
...
# We deliberately allow simple passwords, so we disable the strength
# validators. (On a real site, keep them.)
...
# ---------------------------------------------------------------------------
#  MEDIA — where UPLOADED FILES (candidate resumes) are saved
# ---------------------------------------------------------------------------
# Files uploaded through the API are written under this folder, and served at
# the /media/... URL while DEBUG is on (wired up in jobhub/urls.py).
MEDIA_URL = '/media/'
...
REST_FRAMEWORK = {
    ...
    # Default: anyone can READ, you must log in to WRITE. Individual views
    # tighten this with role-based permission classes (IsRecruiter etc.).
    'DEFAULT_PERMISSION_CLASSES': [
        'rest_framework.permissions.IsAuthenticatedOrReadOnly',
    ],

    'DEFAULT_PAGINATION_CLASS': 'jobhub.pagination.StandardPagination',
    ...
    'DEFAULT_THROTTLE_RATES': {
        'anon': '100/hour',
        'user': '2000/day',
        'apply': '10/day',     # a candidate may apply at most 10 times per day
    },
...
SPECTACULAR_SETTINGS = {
    ...
    'DESCRIPTION': 'A simple, fully working DRF teaching project (JobHub).',
    'VERSION': '1.0.0',
}
