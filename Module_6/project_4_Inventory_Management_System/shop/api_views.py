...
from .serializers import (
    CategorySerializer, ProductSerializer, CartSerializer, AddCartItemSerializer,
...
class CategoryViewSet(viewsets.ReadOnlyModelViewSet):
    """Categories are READ-ONLY through the API (list + retrieve).  (NEW in M6)

    Added so the dashboard's "Add / Edit product" form can fill its category
    dropdown from /api/categories/. It reuses the CategorySerializer that
    already existed in Module 5. Reading is public.
    """
    queryset = Category.objects.all()
    serializer_class = CategorySerializer
    permission_classes = [AllowAny]
    pagination_class = None   # there are only a few categories — send them all
...
class MeView(APIView):
    ...
    JavaScript uses 'is_staff' to decide whether to show the admin-only
    Add / Edit / Delete controls (only staff may change products).

    permission_classes = [AllowAny] so an ANONYMOUS visitor gets a polite 200
    answer ({"is_authenticated": false}) instead of a 401 error.
    """
    permission_classes = [AllowAny]
