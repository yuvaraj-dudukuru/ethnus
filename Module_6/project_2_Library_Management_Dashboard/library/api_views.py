...
class AuthorViewSet(viewsets.ReadOnlyModelViewSet):
    """Authors are READ-ONLY through the API (list + retrieve).

    ADDED in Module 6 so the dashboard's "Author" filter dropdown can be
    populated with the real list of authors. It reuses the AuthorSerializer
    that already existed in Module 5 (it shows id, name and a book_count).
    Reading is public — anyone may fetch the author list to filter by it.
    """
